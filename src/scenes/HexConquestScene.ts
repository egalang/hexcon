...
export class HexConquestScene extends Phaser.Scene {
    ...
    private checkVictory(): boolean {
        ...
        if (this.mode === "campaign") {
            ...
            if (this.isCampaignObjectiveComplete()) {
                ...
                const reward = this.saveCampaignWinResults();

                this.scene.start("CampaignRewardScene", {
            ...
            if (this.hasCampaignObjectiveFailed()) {
                ...
                this.statusText.setText("OBJECTIVE FAILED");
                this.statusText.setColor("#ff8f8f");

                this.clearHighlights();
                this.showEndButtons("red");
        ...
        if (counts.blue === 0 || counts.red === 0) {
            ...
            if (this.mode === "ai") {
                ...
                const rematch = this.createWideButton(
                    WIDTH / 2,
                    585,
                    "Rematch",
                    () => {
                ...
                const setup = this.createWideButton(
                    WIDTH / 2,
                    650,
                    "Change AI Setup",
                ...
                const menu = this.createWideButton(
                    WIDTH / 2,
                    715,
                    "Main Menu",
            ...
            this.countText.setText(
                `Final Score  Blue ${counts.blue}  |  Red ${counts.red}`
            );
        ...
        const movable = [...this.hexes.values()].some(
            h =>
                h.owner === this.currentPlayer &&
                this.neighbors(h).some(n => !n.owner)
        );
        ...
        if (!movable) {
            ...
            this.statusText.setText(
                `NO MOVES — ${winner.toUpperCase()} WINS!`
            );
    ...
    private saveCampaignWinResults(): {
        ...
        saveUnlockedCampaignLevel(
            Math.min(this.campaignLevel + 1, CAMPAIGN_MAX_LEVEL)
        );

        let goldReward = 0;

        // Award gold only the first time this level is completed
        if (!GameData.profile.completedCampaignLevels.includes(this.campaignLevel)) {

            GameData.profile.completedCampaignLevels.push(this.campaignLevel);

            goldReward = this.campaignLevel * 100;

            GameData.profile.gold += goldReward;
    ...
    private showEndButtons(winner: Player) {
        ...
        for (const b of this.endButtons) {
            b.destroy();
        }
        ...
        if (this.mode === 'campaign' && winner === 'red') {
            ...
            const retry = this.createWideButton(
                WIDTH / 2,
                620,
                `Retry Level ${this.campaignLevel}`,
                () => this.restartGame()
            );

            const levels = this.createWideButton(
                WIDTH / 2,
                685,
                'Level Select',
